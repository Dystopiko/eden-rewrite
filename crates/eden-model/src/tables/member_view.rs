...
impl MemberFlags {
    #[must_use]
    pub const fn highest_rank(&self) -> Self {
        if self.contains(Self::ADMINISTRATOR) {
            Self::ADMINISTRATOR
        } else if self.contains(Self::STAFF) {
            Self::STAFF
        } else if self.contains(Self::CONTRIBUTOR) {
            Self::CONTRIBUTOR
        } else {
            Self::REGULAR
        }
...
mod tests {
    ...
    #[sqlx::test(migrations = "../../migrations")]
    async fn should_flags_work_with_members_with_multiple_roles(pool: sqlx::PgPool) {
        let _guard = crate::testing::krate::setup();
